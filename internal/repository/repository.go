...
func (r *Repository) AddVehicle(v *entity.Vehicle) error {
	...
	_, err := r.db.Exec("INSERT INTO vehicles (vin,brand,model,year,status) VALUES ($1,$2,$3,$4,$5)", v.VIN, v.Brand, v.Model, v.Year, v.Status)
	if err != nil {
...
func (r *Repository) GetVehicle(vin string) (*entity.Vehicle, error) {
	...
	var id int
	err := r.db.QueryRow("SELECT * FROM vehicles WHERE vin = $1", vin).Scan(&id, &v.VIN, &v.Brand, &v.Model, &v.Year, &v.Status)
	if err != nil {
...
func (r *Repository) UpdateVehicle(v *entity.Vehicle) error {
	_, err := r.db.Exec("UPDATE vehicles SET brand = $1, model = $2, year = $3, status = $4 WHERE vin = $5", v.Brand, v.Model, v.Year, v.Status, v.VIN)
	if err != nil {
...
func (r *Repository) DeleteVehicle(vin string) error {
	_, err := r.db.Exec("DELETE FROM vehicles WHERE vin = $1", vin)
	if err != nil {
...
func (r *Repository) GetAllVehicles() ([]*entity.Vehicle, error) {
	rows, err := r.db.Query("SELECT * FROM vehicles")
	if err != nil {
...
func (r *Repository) UpdateStatus(vin string, status string) error {
	_, err := r.db.Exec("UPDATE vehicles SET status = $1 WHERE vin = $2", status, vin)
	if err != nil {
...
func (r *Repository) RentVehicle(vin string, u *entity.User) error {
	...
	_, err := r.db.Exec("INSERT INTO users (name,email,phone,reserve_vin,description) VALUES ($1,$2,$3,$4,$5)", u.Name, u.Email, u.Phone, vin, u.Description)
	if err != nil {
